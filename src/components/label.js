...
const Label = () => {
  ...
  const [descriptionBoxPosition, setDescriptionBoxPosition] = useState({ x: 0, y: 0 });
  ...
  const handleMouseUp = () => {
    if (isDrawing) {
      if (currentBox && currentBox.width !== 0 && currentBox.height !== 0) {
        ...
        setDescriptionBoxPosition({ x: currentBox.x, y: currentBox.y - 40 }); // Position the input above the box
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
                      <div className="relative my-4 w-11/12 max-h-screen overflow-auto bg-white p-4 rounded-lg">
                        ...
                        {showDescriptionInput && (
                          <div
                            className="absolute bg-white p-2 border rounded shadow-md"
                            style={{ left: `${descriptionBoxPosition.x}px`, top: `${descriptionBoxPosition.y}px` }}
                          >
                            <input
                              type="text"
